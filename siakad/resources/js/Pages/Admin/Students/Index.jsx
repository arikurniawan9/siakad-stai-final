...
export default function StudentsIndex({ 
    ...
    // Modal Detail Tagihan Keuangan Mahasiswa
    const [billingModalStudent, setBillingModalStudent] = useState(null);
    ...
    return (
        <AppLayout title="Kemahasiswaan">
                    ...
                    <div className="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                        <div>
                            <div className="inline-flex items-center space-x-1.5 px-2.5 py-0.5 rounded-full bg-teal-500/20 text-teal-300 border border-teal-500/30 text-[10px] font-black mb-1">
                                ...
                                <span>{isDosenPa ? 'DOSEN PEMBIMBING AKADEMIK (PA)' : isKaprodi ? 'PROGRAM STUDI & KEMAHASISWAAN' : 'KEMAHASISWAAN & AKADEMIK'}</span>
                            </div>
                            <h2 className="text-base sm:text-lg font-black tracking-tight text-white">
                                {isDosenPa 
                                    ? 'Monitoring Mahasiswa Bimbingan & Tagihan Keuangan' 
                        ...
                        {isSelectionActive ? (
                            <div className="flex items-center gap-1.5 self-start md:self-auto animate-fadeIn">
                                {activeTab === 'students' && (
                                    <>
                                        {/* 1. Tambah Mahasiswa (Hanya Admin & Kaprodi) */}
                                        {!isDosenPa && (
                                        ...
                                        {/* 2. Upload / Import Excel (Hanya Admin & Kaprodi) */}
                                        {!isDosenPa && (
                                            <div className="relative group">
                                                <button
                                                    type="button"
                                                ...
                                                studentList.map((stu, idx) => {
                                                    ...
                                                    return (
                                                        ...
                                                        >
                                                            ...
                                                            {/* Status Tagihan Keuangan & VA BSI */}
                                                            <td className="py-3 px-3 text-center">
                                                                <button
                                                                    type="button"
                                                                    onClick={() => setBillingModalStudent(stu)}
                                                                    className="inline-flex flex-col items-center group cursor-pointer hover:scale-105 transition"
                                                                    title="Klik untuk melihat rincian tagihan keuangan & VA BSI"
                                                                >
                                                                    {stu.billing?.status === 'LUNAS' || stu.invoice_status === 'LUNAS' ? (
                                                                        <span className="px-2 py-0.5 rounded-full text-[9px] font-black bg-emerald-100 text-emerald-800 border border-emerald-300 group-hover:bg-emerald-200 flex items-center space-x-1">
                                                                            <span>✓ Lunas</span>
                                                                        </span>
                                                                    ) : stu.billing?.status === 'SEBAGIAN' ? (
                                                                        <span className="px-2 py-0.5 rounded-full text-[9px] font-black bg-amber-100 text-amber-800 border border-amber-300 group-hover:bg-amber-200 flex items-center space-x-1">
                                                                            <span>⚡ Sebagian</span>
                                                                        </span>
                                                                    ) : (
                                                                        <span className="px-2 py-0.5 rounded-full text-[9px] font-black bg-rose-100 text-rose-800 border border-rose-300 group-hover:bg-rose-200 flex items-center space-x-1">
                                                                            <span>✕ Belum Lunas</span>
                                                                        </span>
                                                                    )}
                                                                    {stu.billing?.remaining_amount > 0 ? (
                                                                        <span className="text-[8.5px] font-bold text-rose-600 font-mono mt-0.5">
                                                                            Sisa: Rp {Number(stu.billing.remaining_amount).toLocaleString('id-ID')}
                                                                        </span>
                                                                    ) : (
                                                                        <span className="text-[8px] font-bold text-slate-400 font-mono mt-0.5">
                                                                            VA: {stu.billing?.va_number || `992802...${stu.id}`}
                                                                        </span>
                                                                    )}
                                                                </button>
                                                            </td>
            ...
            {/* =========================================================================
                MODAL DETAIL TAGIHAN KEUANGAN MAHASISWA & VA BSI (DOSEN PA & ADMIN)
               ========================================================================= */}
            {billingModalStudent && (
                <div 
                    onClick={(e) => {
                        if (e.target === e.currentTarget) setBillingModalStudent(null);
                    }}
                    className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-xs animate-fadeIn"
                >
                    <div className="bg-white rounded-3xl shadow-2xl border border-slate-200 w-full max-w-2xl overflow-hidden animate-in zoom-in-95 duration-150">
                        {/* Header Modal */}
                        <div className="px-6 py-4 bg-gradient-to-r from-slate-900 via-teal-950 to-emerald-950 text-white flex items-center justify-between">
                            <div className="flex items-center space-x-3">
                                <div className="w-10 h-10 rounded-2xl bg-emerald-500/20 text-emerald-400 flex items-center justify-center border border-emerald-500/30">
                                    <CreditCard className="w-5 h-5" />
                                </div>
                                <div>
                                    <h3 className="font-black text-sm text-white">Monitoring Tagihan Keuangan & VA BSI</h3>
                                    <p className="text-[11px] text-teal-200">{billingModalStudent.name} (NIM: {billingModalStudent.identity_number})</p>
                                </div>
                            </div>
                            <button
                                type="button"
                                onClick={() => setBillingModalStudent(null)}
                        ...
                        <div className="p-6 space-y-4 max-h-[80vh] overflow-y-auto">
                            {/* Card Virtual Account BSI */}
                            <div className="p-4 rounded-2xl bg-gradient-to-r from-emerald-50 via-teal-50 to-emerald-50 border border-emerald-200 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                                <div>
                                    <div className="flex items-center space-x-2">
                                        <span className="px-2 py-0.5 rounded text-[9px] font-black bg-emerald-800 text-white">BANK BSI</span>
                                        <span className="text-xs font-bold text-slate-700">Virtual Account Pembayaran</span>
                                    </div>
                                    <div className="text-xl font-black font-mono tracking-wider text-emerald-950 mt-1">
                                        {billingModalStudent.billing?.va_number || `992802${String(billingModalStudent.id).padStart(8, '0')}`}
                                    </div>
                                    <p className="text-[10px] text-slate-500 mt-0.5">Atas Nama: SIAKAD STAI - {billingModalStudent.name}</p>
                                </div>
                                <button
                                    type="button"
                                    onClick={() => {
                                        const va = billingModalStudent.billing?.va_number || `992802${String(billingModalStudent.id).padStart(8, '0')}`;
                                        navigator.clipboard.writeText(va);
                                        setCopiedNim(va);
                                        setTimeout(() => setCopiedNim(null), 2000);
                                    }}
                                    className="px-3 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-bold transition flex items-center justify-center space-x-1.5 shadow-sm cursor-pointer shrink-0"
                                >
                                    {copiedNim ? <Check className="w-3.5 h-3.5" /> : <Copy className="w-3.5 h-3.5" />}
                                    <span>{copiedNim ? 'Tersalin!' : 'Salin Nomor VA'}</span>
                                </button>
                            </div>

                            {/* Ringkasan Finansial KPI */}
                            <div className="grid grid-cols-3 gap-3 text-center">
                                <div className="p-3 bg-slate-50 rounded-2xl border border-slate-200">
                                    <p className="text-[10px] font-bold text-slate-500 uppercase">Total Kewajiban</p>
                                    <p className="text-sm font-black text-slate-900 mt-0.5">
                                        Rp {Number(billingModalStudent.billing?.total_amount || 0).toLocaleString('id-ID')}
                                    </p>
                                </div>
                                <div className="p-3 bg-emerald-50 rounded-2xl border border-emerald-200">
                                    <p className="text-[10px] font-bold text-emerald-700 uppercase">Total Terbayar</p>
                                    <p className="text-sm font-black text-emerald-900 mt-0.5">
                                        Rp {Number(billingModalStudent.billing?.paid_amount || 0).toLocaleString('id-ID')}
                                    </p>
                                </div>
                                <div className="p-3 bg-rose-50 rounded-2xl border border-rose-200">
                                    <p className="text-[10px] font-bold text-rose-700 uppercase">Sisa Tagihan</p>
                                    <p className="text-sm font-black text-rose-900 mt-0.5">
                                        Rp {Number(billingModalStudent.billing?.remaining_amount || 0).toLocaleString('id-ID')}
                                    </p>
                                </div>
                            </div>

                            {/* Rincian Invoice Per Item */}
                            <div>
                                <h4 className="text-xs font-black uppercase tracking-wider text-slate-700 mb-2 flex items-center space-x-1.5">
                                    <span>Daftar Tagihan & Status Pelunasan</span>
                                </h4>
                                {billingModalStudent.billing?.items && billingModalStudent.billing.items.length > 0 ? (
                                    <div className="border border-slate-200 rounded-2xl overflow-hidden shadow-xs">
                                        <table className="w-full text-left text-xs">
                                            ...
                                            <tbody className="divide-y divide-slate-150">
                                                {billingModalStudent.billing.items.map((item, idx) => (
                                                    <tr key={idx} className="hover:bg-slate-50/80">
                                                        <td className="py-2.5 px-3 font-bold text-slate-800">{item.fee_name}</td>
                                                        <td className="py-2.5 px-3 font-mono text-[11px] text-slate-500">{item.invoice_number}</td>
                                                        <td className="py-2.5 px-3 text-right font-black text-slate-900 font-mono">
                                                            Rp {Number(item.amount).toLocaleString('id-ID')}
                                                        </td>
                                                        <td className="py-2.5 px-3 text-center text-[11px] text-slate-500">{item.due_date}</td>
                                    ...
                                    <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 text-center text-xs text-slate-500">
                                        Mahasiswa tidak memiliki tagihan aktif yang belum diselesaikan untuk periode ini.
                                    </div>
                                )}
                            </div>
                        </div>
